...
void EncryptorWidget::outputHandling (bool save)
{
    ...
    else
    {
        ...
        unsigned short int titleBarHeight = QApplication::style()->pixelMetric(QStyle::PM_TitleBarHeight);
        QSize screenSize(QGuiApplication::screens().at(0)->size());
        outputPopup.move(screenSize.width() / 2, 0);
        outputPopup.setFixedSize(QSize(screenSize.width() / 2, screenSize.height() - titleBarHeight));

        QTextEdit ouputPreview;
...
unsigned short int EncryptorWidget::e_LeetSpeak ()
{
    ...
    QString dict(QString::fromStdString(sDict)), leetDict(QString::fromStdString(sLeetDict));
