...
OptionsWidget::OptionsWidget (EncryptorWidget* encryptor) : QWidget ()
{
    ...
    aboutMEncryptorLabel = new QLabel(tr(
        ...
        "under <a href = 'https://www.gnu.org/licenses/gpl-3.0.en.html'>license GPL 3</a> with the framework Qt in C++ language.<br/>"
        "Follow <a href = 'https://qt.io'>this link</a> to learn more about Qt.<br/>"
        "You can also visit <a href = 'https://memetech-inc.weebly.com'>my website</a> to check for updates,<br/>"
        "try other applications or <a href = 'https://github.com/NyanMaths/MEncryptor/issues'>ask for new features</a> !<br/>"
    ...
    uninstallButton = new QPushButton(tr("Uninstall this application"));
    ...
    layout->addWidget(uninstallButton, 1, 0);
...
void OptionsWidget::uninstall ()
{
    if (QFile::exists("unins000.exe"))
    {
        qApp->quit();
        QProcess::startDetached("unins000.exe", QStringList());
    }
    else
        QMessageBox::critical(this, tr("Ooooooops..."), tr("Could not find the uninstaller,\nI guess there is no hope for you...\nJust kidding, you can remove me maually, but it is not really proper."));
}
